...
class StatareaAccumulator:
	def __init__(self, scrape_days=5, update_days=3, db_path="statarea_ml_accumulator.db"):
	...
	def gather_future_matches(self):
		...
		"""
		print(f"Gathering future matches for day {self.scrape_days}. ")
		future_rows = self.scraper.scrape(days=self.scrape_days, parallel=True)
	...
	def update_scores(self):
		...
		"""
		print(f"Updating scores for day {self.update_days}.")
		rows = self.scraper.collect_match_rows(date_links_with_dates=self.scraper.collect_date_links(self.update_days))
		updates = []
		for row in rows:
			updates.append({
				"link": row.get("link"),
	...
	def accumulate_daily(self):
		...
		self.db.log_accumulation(scraped=True, updated=True)
